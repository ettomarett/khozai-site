...
![Premise 4: Subcortical Structures Are Finite and Complete. A cross-section of the brain showing the structures beneath the cortical surface, organized by group: the diencephalon (thalamus, hypothalamus) at center, basal ganglia surrounding it, limbic structures (amygdala, hippocampus) in the temporal lobe, and the brainstem descending below. Each structure is labeled and color-coded by neurochemical system. The inventory is complete.](/images/ch02/ch02_img05_premise4.png)
...
![Premise 5: The Structure-Experience Mapping Is Specific and Graded. Three examples of selective elimination. Left: V1 destroyed - vision gone, but emotion, hearing, cognition, and motivation remain intact. Center: hippocampus destroyed (patient HM) - new memory formation gone, but perception, emotion, and existing knowledge remain intact. Right: dopamine depleted - wanting gone, but liking remains (the double dissociation demonstrated by Berridge & Robinson). Each alteration is surgical: one structure, one aspect, everything else preserved.](/images/ch02/ch02_img06_premise5.png)
...
![Premise 6: The Structure-Behavior Mapping Is Specific and Graded. Penfield's motor homunculus - specific cortex regions mapped to specific body parts. Left: stimulate the hand area, the hand moves. Right: damage Broca's area, speech production stops but comprehension remains. Below: hypothalamic stimulation produces specific physiological changes. The mapping from brain structure to behavioral output is as specific as the mapping from structure to experience.](/images/ch02/ch02_img07_premise6.png)
...
![Experience Space dimension set properties. Four quadrants: Finite (bounded by finite hardware), Resolution-dependent (same dimensions at three zoom levels from 5 broad to 30 narrow), Hierarchical (Vision branches into Color, Motion, Faces - destroying Vision eliminates all children), Discoverable (new dimensions revealed by new selective elimination experiments, growing with evidence not theory).](/images/ch02/ch02_img36_experience_dimensions.png)
...
![Psychological constructs as patterns in Experience Space. Anxiety is not a single dimension but a specific combination: high arousal, negative valence, heightened attention, threat-related cognition, and elevated body-state awareness. Change one component - lower arousal with a beta blocker - and the pattern shifts into something the person no longer calls anxiety. Named emotions are regions in the space, not axes of it.](/images/ch02/ch02_img10b_patterns.png)
...
![Space 4: Behavioral Output Space. Four output systems arranged by timescale: motor (milliseconds), autonomic (seconds), endocrine (minutes-hours), immune (hours-days). Self-report (a special case of motor output) bridges to Experience Space - the words are behavioral output, but their content references subjective experience. The space is fully objective: every dimension can be measured by a third-party observer.](/images/ch02/ch02_img11_space4.png)
...
![Mapping 3: Structural Inference. The logic of double dissociation: if destroying structure A eliminates function X but preserves function Y, and destroying structure B eliminates Y but preserves X, then X and Y must be produced by separable systems. This inference - from neural architecture to experiential architecture - is the mapping the framework actually uses. It preserves independence, hierarchy, and gradedness. It does not preserve content: knowing that V1 produces a visual dimension does not tell us what seeing looks like.](/images/ch02/ch02_img12c_inference.png)
...
![How Vc is evaluated: the school exam analogy. Give the model and a human expert the same task - describe an image, answer a question, transcribe audio - and compare scores. The human baseline is expert performance on the same test. This measures whether the model arrives at the same meaning, regardless of how it gets there internally.](/images/ch02/ch02_img14_vc_exam.png)
...
![Vc understanding benchmarks across four modalities, showing a clear confidence gradient from strongest to weakest. Text: model scores 90% versus human 89.8% on MMLU - near parity, high confidence. Images: model 85-86% versus human 88.6% on MMMU - a 3-point gap, high confidence. Audio: model 59.2% versus human 77.9% on MMAU-Pro - a 19-point gap, moderate confidence. Video: model 49.4% versus human 90.7% on Video-MME-v2 - a 41-point gap, low confidence. Only Gemini processes video natively; other models sample frames.](/images/ch02/ch02_img30_vc_understanding_gradient.jpg)
...
![Video processing gap. Left: Gemini processes video natively with temporal flow intact. Right: GPT and Claude sample individual frames, losing temporal continuity. The 41-point performance gap (49.4% vs 90.7% human on Video-MME-v2) reflects both model capability and whether the model actually watches the video or just looks at snapshots.](/images/ch02/ch02_img39_video_processing_gap.png)
